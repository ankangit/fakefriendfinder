...
class UsersController extends Controller
{
	...
	public function actionForgotPassword()
	{
        ...
        if(isset($_POST['PasswordReset']))
		{
			...
			//$user_model=Users::model()->findByAttributes(array('email'=>$email, 'username'=>$username));
                        $sql="select * from users where username='".$username."' or email='".$username."'";
                        $user_model=Users::model()->findBySql($sql);
           	if($user_model==null)
			{
			...
			else
			{
                      ...
                      Yii::import('application.extensions.phpmailer.JPhpMailer');
						...
						$user_model->update();
						 ...
						 //------------
						  $mail->Subject = $emailTemplate->email_template_subject;
						 $ls_emailBody = $this->makeForgotPasswordEmailBody($user_model,$link,$m);
    ...
    public function makeForgotPasswordEmailBody($user_model,$url,$v)
	{
		...
		$ls_bodyContent = str_replace('[%LINK%]', $url, $ls_bodyContent);
                $ls_bodyContent = str_replace('[%CODE%]', $user_model->activation_key, $ls_bodyContent);
                $ls_bodyContent = str_replace('[%PASSWORD%]', $user_model->password, $ls_bodyContent);
